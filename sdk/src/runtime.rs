...
impl GeneratorMetadata {
    ...
    fn generate(&self, engine_dmmf: EngineDMMF) -> Result<(), GeneratorError> {
        ...
        let internal_data_model = prisma_models::convert(schema.clone(), "".into());
        let query_schema = Arc::new(schema_builder::build(internal_data_model.clone(), true));
        let dmmf = from_precomputed_parts(query_schema.clone());
